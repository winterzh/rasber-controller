...
class PrivateProtocolV2026(ProtocolBase):
    """
    ...
    NAME = "PRIVATE_V2026"
    ADDR_MIN = 1
    ADDR_MAX = 0xFFFFFFFE  # 4-byte uint, 留出 0 / 0xFFFFFFFF 作非法值
    SCAN_MAX = 1023        # AutoID 上限
    ...
    def scan_address(self, auto_id: int, timeout_ms: int = 300) -> dict:
        ...
        # send并recv (sync)
        response = self.driver.send_and_receive(
            command_frame, response_size=12, timeout_ms=timeout_ms, expected_bytes=12
        )
