...
def send_report_via_modem(config, modem):
    """通过 4G modem 发送报告 (独立连接到硬编码 broker)

    流程：断开数据 MQTT → 连接 report broker → 发送 → 断开
    """
    try:
        ...
        # 断开当前数据 broker
        modem.disconnect_mqtt()

        # 连接到 controller-manager broker
        modem.send_at("AT+CMQTTSTART", 5000)
        modem.send_at('AT+CMQTTACCQ=0,"ESP32_Report"', 5000)
        ok, _ = modem.send_at(
            f'AT+CMQTTCONNECT=0,"tcp://{_REPORT_BROKER}:{_REPORT_PORT}",60,1',
            30000, "OK"
        )
        if not ok:
            print("[Report] 4G MQTT connect failed")
            return False

        # 发送
        ok = modem.publish(_REPORT_TOPIC, payload)
        if ok:
        ...
        # 断开 report broker
        modem.send_at("AT+CMQTTDISC=0,60", 5000)
        modem.send_at("AT+CMQTTREL=0", 5000)
        modem.send_at("AT+CMQTTSTOP", 5000)
        modem._mqtt_connected = False
