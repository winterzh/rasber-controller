...
# 复制 Adafruit 库
ADAFRUIT_LIB="$(cd "$(dirname "$SRC_DIR")/.." && pwd)/ref/adafruit-circuitpython-bundle-9.x-mpy-20260204/lib"
if [ -d "$ADAFRUIT_LIB" ]; then
    echo "📦 复制 Adafruit 库..."
    # 目录型库
    cp -r "$ADAFRUIT_LIB/asyncio" "$DST_DIR/lib/"
    cp -r "$ADAFRUIT_LIB/adafruit_ble" "$DST_DIR/lib/"
    cp -r "$ADAFRUIT_LIB/adafruit_minimqtt" "$DST_DIR/lib/"
    cp -r "$ADAFRUIT_LIB/adafruit_wiznet5k" "$DST_DIR/lib/"
    # 单文件库
    cp "$ADAFRUIT_LIB/adafruit_ticks.mpy" "$DST_DIR/lib/"
    cp "$ADAFRUIT_LIB/adafruit_ntp.mpy" "$DST_DIR/lib/"
    cp "$ADAFRUIT_LIB/adafruit_requests.mpy" "$DST_DIR/lib/"
    cp "$ADAFRUIT_LIB/adafruit_connection_manager.mpy" "$DST_DIR/lib/"
    echo "   ✓ asyncio, adafruit_ble, adafruit_minimqtt, adafruit_wiznet5k"
    echo "   ✓ adafruit_ticks, adafruit_ntp, adafruit_requests, adafruit_connection_manager"
else
    echo "⚠️  未找到 Adafruit 库目录: $ADAFRUIT_LIB"
fi
